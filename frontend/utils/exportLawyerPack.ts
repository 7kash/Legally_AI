...
export async function exportLawyerPackToPDF(options: LawyerPackOptions): Promise<void> {
  ...
  doc.setFontSize(10)
  doc.setFont('helvetica', 'bold')
  doc.text('⚠️  IMPORTANT NOTICE', margin + 5, yPosition)
  yPosition += 7
  doc.setFont('helvetica', 'normal')
  ...
  if (analysisData?.screening_result) {
    ...
    const screeningMap: Record<string, string> = {
      no_major_issues: '✓ No major issues identified',
      recommended_to_address: '⚠️  Issues recommended to address before signing',
      high_risk: '🚨 High-risk terms identified - legal review strongly recommended',
      preliminary_review: 'ℹ️  Preliminary review - verification recommended',
    }
  ...
  if (content.payment_terms?.content) {
    ...
    if (typeof terms === 'object' && !Array.isArray(terms)) {
      Object.entries(terms).forEach(([key, value]) => {
        if (value) {
          addText(`• ${key.replace(/_/g, ' ')}: ${value}`, 10)
        }
      })
    }
    yPosition += 5
